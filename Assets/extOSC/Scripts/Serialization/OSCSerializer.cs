...
namespace extOSC.Serialization
{
    public static class OSCSerializer
    {
        ...
        public static T Deserialize<T>(OSCMessage message)
        {
            ...
            return (T)Unpack(message.Values, ref start, typeof(T));
        }

        public static void Pack(List<OSCValue> values, object value)
        {
            if (value == null) 
                return;

            var packer = GetPacker(value.GetType());
            if (packer != null)
            {
                packer.Pack(values, value);
            }
        }
