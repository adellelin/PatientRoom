...
namespace extOSC.Editor.Panels
{
    ...
    public class OSCSplitPanel : OSCPanel
    {
        ...
        public OSCSplitPanel(OSCWindow parent, string uniqueName, SplitOrientation orientation = SplitOrientation.Horizontal) : base(parent, uniqueName)
        {
        ...
        protected override void DrawContent(Rect contentRect)
        {
            ...
            foreach (var data in _storedPanelData)
            {
                ...
                panel.SetContentRect(panelRect);
                panel.Draw();
                ...
                switch (Event.current.type)
                {
                    ...
                    case EventType.MouseDrag:
                        if (_selectedSplitter != null)
                        {
                            ...
                            if (_orientation == SplitOrientation.Horizontal)
                            {
                                pixelSize = _selectedSplitter.FirstPanel.ContentRect.width +
                                            _selectedSplitter.SecondPanel.ContentRect.width;

                                newPixelSize = _selectedSplitter.FirstPanel.ContentRect.width + Event.current.delta.x;
                            }
                            else if (_orientation == SplitOrientation.Vertical)
                            {
                                pixelSize = _selectedSplitter.FirstPanel.ContentRect.height +
                                            _selectedSplitter.SecondPanel.ContentRect.height;

                                newPixelSize = _selectedSplitter.FirstPanel.ContentRect.height + Event.current.delta.y;
                            }
        ...
        private void LoadData()
        {
            ...
            foreach (var storedData in _storedPanelData)
            {
                var key = string.Format("osc.editor.splitPanel.{2}.{0}.{1}", UniqueName, storedData.Panel.UniqueName, _parent.GetType());
